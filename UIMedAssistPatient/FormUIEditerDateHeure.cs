...
{
    public partial class FormUIEditerDateHeure : Form
    {
        ...
        private int idPlanningMed;
        public FormUIEditerDateHeure(DateTime date, int idMedecin, int idMaison, int idSpecialite, int idConsultation, int idRDV, TimeSpan anHeure)
        {
            InitializeComponent();
            onLoadDate(idMedecin,idMaison);
            this.date = date;
        ...
        private void onLoadDate(int idmedecin,int idmaison)
        {
            try
            {
                ...
                dataGridViewDate.DataSource = list1;
        ...
        {
            flowLayoutPanelHeure.Controls.Clear();
            string d = dataGridViewDate.Rows[e.RowIndex].Cells[1].Value.ToString();
            this.date = DateTime.Parse(d);
            string idpl= dataGridViewDate.Rows[e.RowIndex].Cells[0].Value.ToString();
            this.idPlanningMed = int.Parse(idpl);
            BLPatient.blPatient blPatient = new BLPatient.blPatient();
        ...
        private void btValiderEditionRDV_Click(object sender, EventArgs e)
        {
            BLPatient.blPatient blPatient = new BLPatient.blPatient();
            ...
            RDV.idPlanningMed = this.idPlanningMed.ToString();
            listRDV.Add(RDV);
            string Json = JsonConvert.SerializeObject(listRDV, Formatting.Indented);
            blPatient.UpdateRDV1(Json);
        ...
        private async void btSaveAPI_Click(object sender, EventArgs e)
        {
            ...
            RDV1.idPlanningMed = this.idPlanningMed.ToString();
            listRDV1.Add(RDV1);
            ...
            var response1 = await httpClient.PostAsync("https://localhost:44399/Medecin/UpdateAllPlanning/", content1);
